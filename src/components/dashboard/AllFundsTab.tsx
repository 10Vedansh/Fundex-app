...
function getSubCategory(fund: MutualFund): string {
  ...
  // For numeric/unrecognized category IDs (Equity workbook uses raw IDs), infer from fund name
  const isNumeric = /^\d+$/.test(cat);
  if (isNumeric || fund.assetClass === 'Equity') {
    const name = (fund.name || '').toLowerCase();
    if (/\b(?:large|bluechip)\b/.test(name)) return 'Large Cap';
    if (/\bmid[\s-]?cap\b/.test(name)) return 'Mid Cap';
    if (/\bsmall[\s-]?cap\b/.test(name)) return 'Small Cap';
    if (/\bflexi[\s-]?cap\b/.test(name)) return 'Flexi Cap';
    if (/\bmulti[\s-]?cap\b/.test(name)) return 'Multi Cap';
    if (/\belss\b/.test(name) || /\btax[\s-]?sav(?:er|ing)\b/.test(name)) return 'ELSS';
    if (/\b(?:index|etf|nifty|sensex|bse)\b/.test(name)) return 'Index Funds';
    if (/\b(?:international|global|us|world|foreign|overseas)\b/.test(name)) return 'International';
